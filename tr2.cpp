...
void Mil(double [N+1],double [N], double [N+1], double [2], double [2]);

void P_Mil_Fine(int, double [Nc+1], double [Nc+1], double [N+1],double [N], double [N+1], double [2], double [2], int);
void P_Mil_Coarse(double [N+1], double [Nc+1], double, double [2], double [2], int);
void P_Mil_er_pro(double [Nc+1], double [N+1], double [Nc+1], double [Nc+1], int, double, double [2], double [2], double [N+1]);
...
{  
	...
	/*   **********************************************************************************************
	  P_Mil_Coarse(W, Yc, Dtc, a, b, NNc);	  
	  // P_Naive_Eu_Coarse(W, Yc, Dtc, a, b, NNc);

	   if (myrank == 0){
			int i0c = myrank;
		    // for (ic=0; ic < Nc; ic++){			  
			P_Mil_Fine(i0c, Yc, Yc2, W, dW, Y, a, b, NNc);
	   }
	  ...
	  for (int iu = 0; iu<2; iu++){
		  ...
		  if (myrank != 0){
			  int icc = myrank;
			  // for (ic=0; ic < Nc; ic++){			  
			  P_Mil_Fine(icc, Yc, Yc2, W, dW, Y, a, b, NNc);
		  ...
		  P_Mil_er_pro(Yc, Y, Yc2, Sc, NNc, Dtc, a, b, W);
		   }
	  ...
	  if (myrank  != 0){
		  int imc = myrank;
		  // for (ic=0; ic < Nc; ic++){	
		  P_Mil_Fine(imc, Yc, Yc2, W, dW, Y, a, b, NNc);
		  //P_Naive_Eu_Fine(Min, ic, Yc, Yc2, W, dW, Y, a, b, NNc);
	  }
  ...
  // }
	  ...
	  */
	    ...
	    //////  Time Output     ////////////////////////////////////////////////////
	    ////////////////////////////////////////////////////////////////////////
		  final_time = MPI_Wtime() - init_time;

          if (myrank ==0){	
		  ofstream miafile;
		  miafile.open ("Time.txt");	
		     miafile <<  final_time <<  endl;
		    miafile.close();
	   }
	  //cout << myrank << endl; 
	...
	void BM_SK(const double scale, double dW[N], double W[N+1]){
	  ...
	  void Eu(double W[N+1],double dW[N], double Y[N+1], double a[2], double b[2]){ 
		 ...
		 Y[i+1] = Y[i] +  (a[j]* Y[i]*dt) + (b[j]* Y[i]* dW[i]); // dW[i] = W[i+1] - W[i]; this is Delta_W step (ii-2) of the approximation of Y[i+1]
				// X[i+1] = X[0] + (a[j] *i*dt) + (b[j]*W[i+1]); //i*dt = t 
				}			
	  }
	 ...
	 void P_Eu_Coarse(double W[N+1], double Yc[Nc+1], double Dtc, double a[2], double b[2], int NNc){
	      int j= 0;
		  for(int i=0; i< Nc; i++){
			Yc[i+1] = Yc[i] +  (a[j]* Yc[i]*Dtc) + (b[j]* Yc[i]* (W[(i+1)*NNc]-W[i*NNc])); // dW[i] = W[i+1] - W[i]; this is Delta_W step (ii-2) of the approximation of Y[i+1]
			// X[i+1] = X[0] + (a[j] *i*dt) + (b[j]*W[i+1]); //i*dt = t 
		  }
	  }
	 ...
	 void P_er_pro(double Yc[Nc+1], double Y[N+1], double Yc2[Nc+1], double Sc[Nc+1], int NNc, double Dtc, double a[2], double b[2], double W[N+1]){
		 ...
		 int j= 0;
		  for(int i=0; i<Nc; i++){
				Yc[i+1] = Yc[i] +  (a[j]* Yc[i]*Dtc) + (b[j]* Yc[i]*(W[(i+1)*NNc]-W[i*NNc]))+ Sc[i+1]; // dW[i] = W[i+1] - W[i]; this is Delta_W step (ii-2) of the approximation of Y[i+1]
				// X[i+1] = X[0] + (a[j] *i*dt) + (b[j]*W[i+1]); //i*dt = t 
		  }
	 }

		  void Mil(double W[N+1],double dW[N], double Y[N+1], double a[2], double b[2]){ 
		   int j= 0;
	       for(int i=0; i< N; i++){		       	 
		 						
		 Y[i+1] = Y[i] +  (a[j]* Y[i]*dt) + (b[j]* Y[i]* dW[i])+ (0.50*b[j]*b[j]* Y[i]* ((dW[i]*dW[i])-dt))  ; // dW[i] = W[i+1] - W[i]; this is Delta_W step (ii-2) of the approximation of Y[i+1]
				// X[i+1] = X[0] + (a[j] *i*dt) + (b[j]*W[i+1]); //i*dt = t 
				}			
	  }
	 
      void P_Mil_Fine(int ic, double Yc[Nc+1], double Yc2[Nc+1], double W[N+1],double dW[N], double Y[N+1], double a[2], double b[2], int NNc){
		   int j= 0;
		   Y[ic*NNc] = Yc[ic];
		   for(int i=(ic*NNc); i< (ic+1)*NNc; i++){		       	 
	 ...
	 void P_Mil_Coarse(double W[N+1], double Yc[Nc+1], double Dtc, double a[2], double b[2], int NNc){
	      int j= 0;
		  for(int i=0; i< Nc; i++){
			Yc[i+1] = Yc[i] +  (a[j]* Yc[i]*Dtc) + (b[j]* Yc[i]* (W[(i+1)*NNc]-W[i*NNc]))+ (0.50*b[j]*b[j]* Yc[i]* (((W[(i+1)*NNc]-W[i*NNc])*(W[(i+1)*NNc]-W[i*NNc]))-Dtc)); // dW[i] = W[i+1] - W[i]; this is Delta_W step (ii-2) of the approximation of Y[i+1]
			// X[i+1] = X[0] + (a[j] *i*dt) + (b[j]*W[i+1]); //i*dt = t 
		  }
	  }

	 void P_Mil_er_pro(double Yc[Nc+1], double Y[N+1], double Yc2[Nc+1], double Sc[Nc+1], int NNc, double Dtc, double a[2], double b[2], double W[N+1]){
		 for (int i=1; i< (Nc+1); i++){
			 Sc[i] = Sc[i] + (Yc2[i] - Yc[i]);
		 }
		 int j= 0;
		  for(int i=0; i<Nc; i++){
				Yc[i+1] = Yc[i] +  (a[j]* Yc[i]*Dtc) + (b[j]* Yc[i]*(W[(i+1)*NNc]-W[i*NNc])) + (0.50*b[j]*b[j]* Yc[i]* (((W[(i+1)*NNc]-W[i*NNc])*(W[(i+1)*NNc]-W[i*NNc]))-Dtc)) + Sc[i+1]; // dW[i] = W[i+1] - W[i]; this is Delta_W step (ii-2) of the approximation of Y[i+1]
				// X[i+1] = X[0] + (a[j] *i*dt) + (b[j]*W[i+1]); //i*dt = t 
		  }
